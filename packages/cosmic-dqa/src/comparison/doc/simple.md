```san export=preview caption=简单结构
...
export default class DefaultDemo extends Component {
    ...
    static template = `
        <div>
            <h4>无比对详情列表</h4>
            <cosd-comparison linkInfo="{{linkInfo}}" targets="{{targets}}" bar="{{bar}}"/>
            <h4>无bar</h4>
            <cosd-comparison linkInfo="{{linkInfo}}" targets="{{targets}}" items="{{items}}" />
            <h4>无链接</h4>
            <cosd-comparison targets="{{targets}}" items="{{items}}" />
        </div>
    `;
